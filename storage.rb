...
class Storage
  ...
  def parse_rentals(new_rentals)
    ...
    JSON.parse(File.read(file_name)).map do |rental|
      ...
      new_rentals.rentals << Rental.new(rental['date'], @person, @book) if @book != 'null' && @person != 'null'
    end
  end
  ...
  def save_rentals(new_rentals)
    ...
    new_rentals.rentals.each do |rental|
      @json_rentals.push({ 'json_class' => 'Rental', 'date' => rental.date, 'person' => rental.person.id,
